...
function x(a){var b=__turbopack_require__(a);if("function"!==typeof b.then||"fulfilled"===b.status)return null;b.then(function(c){b.status="fulfilled";b.value=c},function(c){b.status="rejected";b.reason=c});return b}function aa(){}
function ba(a){for(var b=a[1],c=[],d=0;d<b.length;d++){var f=b[d],h=w.get(f);if(void 0===h){h=__turbopack_load__(f);c.push(h);var n=w.set.bind(w,f,null);h.then(n,aa);w.set(f,h)}else null!==h&&c.push(h)}return 4===a.length?0===c.length?x(a[0]):Promise.all(c).then(function(){return x(a[0])}):0<c.length?Promise.all(c):null}
